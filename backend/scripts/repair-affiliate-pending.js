...
const replacement = [
  ...
  "      where: { marketplace: 'MERCADOLIVRE', affiliateUrl: null, productUrl: { not: null } },",
  '      select: {',
  '        id: true, externalProductId: true, title: true, price: true, discountPercent: true,',
  '        soldQuantity: true, imageUrl: true, productUrl: true, categoryId: true, categoryName: true, updatedAt: true,',
  "        scores: { select: { score: true, calculatedAt: true }, orderBy: { calculatedAt: 'desc' }, take: 1 },",
  '      },',
  '      take: 100,',
  '    });',
  '    const pending = candidates',
  "      .filter(p => { const url = String(p.productUrl || ''); return /^https:\\/\\/(?:www\\.)?mercadolivre\\.com\\.br\\/.+$/i.test(url) && !/^https:\\/\\/produto\\.mercadolivre\\.com\\.br\\/MLB\\d+$/i.test(url); })",
  '      .map(p => ({',
  '        id: p.id, externalProductId: p.externalProductId, title: p.title, price: Number(p.price || 0),',
  '        discountPercent: Number(p.discountPercent || 0), soldQuantity: p.soldQuantity, imageUrl: p.imageUrl,',
  '        productUrl: p.productUrl, categoryId: p.categoryId, categoryName: p.categoryName, updatedAt: p.updatedAt,',
  "        score: p.scores[0]?.score ?? null, affiliateStatus: 'PENDING',",
  '      }))',
  '      .sort((a, b) => {',
  '        const scoreA = Number(a.score ?? -1), scoreB = Number(b.score ?? -1);',
  '        if (scoreA !== scoreB) return scoreB - scoreA;',
  '        const salesA = a.soldQuantity ?? -1, salesB = b.soldQuantity ?? -1;',
