...
export class ProductHunterService {
  ...
  async search(query: string, userId?: string) {
    ...
    let data:any;
    if(userId) data=await this.mercadoLivre.searchCatalog(userId, query);
    else { const response=await axios.get('https://api.mercadolibre.com/sites/MLB/search',{params:{q:query.trim(),limit:20},headers:{'Accept':'application/json','User-Agent':'ML-Affiliate-AI/1.0'},timeout:15000}); data=response.data; }
