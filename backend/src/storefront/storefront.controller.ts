...
export class StorefrontController {
  ...
  async product(@Param('id') id: string, @Res() res: Response) {
    ...
    if (!p || !p.affiliateUrl) throw new NotFoundException('AFFILIATE_PRODUCT_NOT_AVAILABLE');
  ...
  async go(@Param('id') id: string, @Res() res: Response) {
    ...
    if (!p?.affiliateUrl || !/^(https:\/\/)?(www\.)?(meli\.la|mercadolivre\.com\.br)\//i.test(p.affiliateUrl)) throw new NotFoundException('SAFE_AFFILIATE_LINK_UNAVAILABLE');
  ...
  async sitemap(@Res() res: Response) {
    ...
    const products = await this.prisma.product.findMany({ where: { affiliateUrl: { not: null } }, select: { id: true, updatedAt: true }, take: 5000, orderBy: { updatedAt: 'desc' } });
